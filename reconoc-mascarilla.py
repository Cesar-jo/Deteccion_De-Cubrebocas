...
face_mask.read(r"C:\Users\ternu\OneDrive\Escritorio\reconocimiento_mascarilla\face_mask_model.xml") #leemos el modelo entrenado con la funcion read
...
#el primer numero sera a la etiqueta que le coresponde = ejmeplo si sale (1,numeros aleatorios) si es uno sera que corresponde a sin mascarilla
#si da = (0, numeros aleatorios) es que corresponde a personas con mascarillas
                cv2.putText(frame, "{}".format(result), (xmin, ymin - 5), 1, 1.3, (210, 124, 176), 1, cv2.LINE_AA)
        
#vamos ahora a poner en verde si se detecta mascarilla y en rojo si no se detecta mascarilla
#eso lo hacemos gracias a las predicciones ya que nos arrojo 1 que es sin mascarilla y 0 con mascarilla
                if result[1] < 150: #si resultado es 1 que nos muestre el rectangulo en color verde
